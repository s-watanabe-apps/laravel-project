<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Authentication Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used during authentication for various
    | messages that we need to display to the user. You are free to modify
    | these language lines according to your application's requirements.
    |
    */

    'confirm' => '確認',
    'edit' => '編集',
    'save' => '保存',
    'edit' => '編集',
    'cancel' => 'キャンセル',
    'do_confirm' => '確認する',
    'delete' => '削除',
    'deleted' => '削除する',
    'choose_file' => 'ファイル選択',
    'file_delete' => 'ファイル削除',
    'email' => 'Eメール',
    'datetime' => '日時',
    'category' => 'カテゴリ',
    'title' => 'タイトル',
    'body' => '本文',
    'contributor' => '投稿者',
    'contribute_date' => '投稿日',
    'weekly_calendar' => '週間カレンダー',
    'monthry_calendar' => '月間カレンダー',
    'profile' => 'プロフィール',
    'profile_edit' => 'プロフィール編集',
    'name' => '名前',
    'name_kana' => 'フリガナ',
    'birth_date' => '生年月日',
    'profile_result_message' => 'プロフィールを更新しました。',
    'article_body' =>'投稿本文',
    'latest_articles' => '直近の記事',
    'pictures' => 'アルバム',
    'show_more' => 'もっと見る',
    'user_name' => 'ユーザー名',
    'password' => 'パスワード',
    'enable' => '有効',
    'disable' => '無効',
    'enabled' => '有効にする',
    'disabled' => '無効にする',
    'none' => 'なし',
    'group' => 'グループ',
    'created_at' => '作成日時',
    'updated_at' => '更新日時',
    'last_login' => '最終ログイン',
    'status' => 'ステータス',
    'invitation' => '招待状',

    // Side menus
    'write_articles' => '記事を書く',
    'pictures' => 'アルバム',
    'members' => 'メンバーリスト',
    'member_search' => 'メンバー検索',
    'community_search' => 'コミュニティ検索',
    'admin_menus' => '管理者メニュー',
    'common_settings' => '共通設定',
    'site_settings' => 'サイト設定',
    'contents_management' => 'コンテンツ管理',
    'informations_management' => 'お知らせ管理',
    'user_management' => 'ユーザー管理',
    'group_management' => 'グループ管理',

    // Messages
    'message' => 'メッセージ',
    'message_center' => '新着メッセージ',
    'readmore' => 'もっと見る',
    'inbox' => '受信BOX',
    'outbox' => '送信BOX',
    'garbage' => 'ゴミ箱',
    'subject' => '件名',
    'body' => '本文',
    'message_from' => '送信者',
    'received_time' => '受信日時',
    'message_from_user' => '%s さんからのメッセージ',
    'reply' => '返信',
    'send_message' => 'メッセージを送る',

    // Favorites
    'favorites' => 'お気に入り',
    'add_favorites' => 'お気に入りに追加しました。',
    'remove_favorites' => 'お気に入りから削除しました。',
    'someones_profile' => '%sのプロフィール',

    // Operation messages.
    'operation_messages' => [
        'profile_update' => 'プロフィールを更新しました。',
        'add_favorites' => 'お気に入りに追加しました。',
        'remove_favorites' => 'お気に入りから削除しました。',
        'saved_site_settings' => 'サイト設定を保存しました。',
    ],

    // Error messages.
    'errors' => [
        'fatal' => '予期せぬエラーが発生しました。はじめからやり直してください。<br>改善しない場合はシステム管理者へ連絡してください。',
        'validation' => '入力値が不正です。<br>改善しない場合はシステム管理者へ連絡してください。',
    ],

    // Roles
    'roles' => [
        'admin' => '管理者',
        'member' => 'メンバー',
    ],

    // Input types
    'input_types' => [
        'fillin' => 'Fill in',
        'description' => 'Description',
        'choice' => 'Choice',
        'select' => 'Select',
    ],

    // DataTables (ja only)
    'datatables' => [
        "sEmptyTable" => "テーブルにデータがありません",
        "sInfo" => " _TOTAL_ 件中 _START_ から _END_ まで表示",
        "sInfoEmpty" => " 0 件中 0 から 0 まで表示",
        "sInfoFiltered" => "（全 _MAX_ 件より抽出）",
        "sInfoThousands" => ",",
        "sLengthMenu" => "_MENU_ 件表示",
        "sLoadingRecords" => "読み込み中...",
        "sProcessing" => "処理中...",
        "sSearch" => "検索:",
        "sZeroRecords" => "一致するレコードがありません",
        "oPaginate" => [
            "sFirst" => "先頭",
            "sLast" => "最終",
            "sNext" => "次",
            "sPrevious" => "前"
        ],
        "oAria" => [
            "sSortAscending" => ": 列を昇順に並べ替えるにはアクティブにする",
            "sSortDescending" => ": 列を降順に並べ替えるにはアクティブにする"
        ]
    ],

    // Managements Settings
    'basic_settings' => '基本設定',
    'site_name' => 'サイト名',
    'site_description' => 'サイト説明',
    'user_create' => 'ユーザー登録',
    'user_create_any' => '誰でも登録できる',
    'user_create_member' => 'メンバーの招待で登録できる',
    'user_create_admin' => '管理者の招待で登録できる',
    'basic_auth' => 'ベーシック認証',
    'user_list' => 'ユーザー一覧',
    'add_user' => 'ユーザー作成',
    'information_list' => 'お知らせ一覧',
    'add_information' => 'お知らせ作成',
    'start_time' => '表示開始日時',
    'end_time' => '表示終了日時',

];
